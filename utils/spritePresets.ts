/**
 * ============================================================================
 * МОДУЛЬ: 20 ДИЗАЙНЕРСКИХ ПАЛИТР И СПРАЙТЫ ХОМЯКА-БУХАНОЧКИ (Horizontal Loaf)
 * ============================================================================
 * 
 * 🎓 ИНТЕРАКТИВНЫЙ УЧЕБНИК: АНАТОМИЯ ГОРИЗОНТАЛЬНОГО ХОМЯКА
 * ----------------------------------------------------------------------------
 * 1. ПОЧЕМУ ХОМЯК ДОЛЖЕН БЫТЬ ВЫТЯНУТ В ШИРИНУ:
 *    Настоящие хомячки при ходьбе и сидении не стоят вертикально, как суслики,
 *    а представляют собой приземистый горизонтальный овал (форму батончика или буханочки,
 *    "hamster loaf").
 *    - Ширина тела: 22-23 пикселя.
 *    - Высота тела: всего 10-12 пикселей (с лапками и ушками ~14-15 пикселей).
 *    - Низкая посадка к опилкам, плавный изгиб спины, маленькие лапки снизу.
 * ============================================================================
 */

import { HamsterPalette, FoodItem } from '@/types/hamster';

export const HAMSTER_PALETTES: HamsterPalette[] = [
  {
    id: 'classic_golden',
    name: 'Золотистый сириец',
    nameEn: 'Classic Golden',
    description: 'Классический пушистый золотисто-рыжий окрас со светлым животиком.',
    fur: '#e69138',
    furDark: '#994d00',
    furLight: '#ffbe76',
    belly: '#fff2cc',
    pink: '#f8bbd0',
    eyes: '#1b1b1b',
    eyeHighlight: '#ffffff',
    cheeks: '#ff85a2',
    paws: '#ffcdd2',
  },
  {
    id: 'roborovski_sand',
    name: 'Песочный Роборовского',
    nameEn: 'Roborovski Sand',
    description: 'Нежный песочно-палевый оттенок крошечного пустынного хомячка.',
    fur: '#dfb77c',
    furDark: '#99733e',
    furLight: '#fae3c3',
    belly: '#fcf6ed',
    pink: '#f8d7da',
    eyes: '#201b15',
    eyeHighlight: '#ffffff',
    cheeks: '#f1b0b7',
    paws: '#fcebe0',
  },
  {
    id: 'albino_snow',
    name: 'Белоснежный альбинос',
    nameEn: 'Albino Snow',
    description: 'Ослепительно белая шубка с рубиново-красными глазками.',
    fur: '#f5f5f5',
    furDark: '#d0d0d0',
    furLight: '#ffffff',
    belly: '#ffffff',
    pink: '#ffccd5',
    eyes: '#c9184a',
    eyeHighlight: '#ff758f',
    cheeks: '#ffb3c1',
    paws: '#ffe5ec',
  },
  {
    id: 'panda_piebald',
    name: 'Черно-белый панда',
    nameEn: 'Panda Piebald',
    description: 'Контрастный пятнистый окрас с темной маской и белым тельцем.',
    fur: '#2b2d42',
    furDark: '#141521',
    furLight: '#535878',
    belly: '#edf2f4',
    pink: '#ffb5a7',
    eyes: '#0d0d0d',
    eyeHighlight: '#ffffff',
    cheeks: '#fcd5ce',
    paws: '#f8edeb',
  },
  {
    id: 'caramel_cream',
    name: 'Теплый карамельный',
    nameEn: 'Caramel Cream',
    description: 'Аппетитный мягкий оттенок ириски с кремовым подшерстком.',
    fur: '#c68b59',
    furDark: '#7a451d',
    furLight: '#f0c299',
    belly: '#ffe8d6',
    pink: '#f7cad0',
    eyes: '#301b0f',
    eyeHighlight: '#ffffff',
    cheeks: '#f4acb7',
    paws: '#fed9b7',
  },
  {
    id: 'charcoal_dark',
    name: 'Глубокий угольный',
    nameEn: 'Charcoal Dark',
    description: 'Загадочный антрацитовый окрас с серебристым отливом.',
    fur: '#3a3a3c',
    furDark: '#1a1a1c',
    furLight: '#68686e',
    belly: '#636366',
    pink: '#d8b4b8',
    eyes: '#000000',
    eyeHighlight: '#ffffff',
    cheeks: '#a37081',
    paws: '#8e8e93',
  },
  {
    id: 'cinnamon',
    name: 'Коричный',
    nameEn: 'Cinnamon',
    description: 'Пряный коричневато-медный цвет корицы с бежевым брюшком.',
    fur: '#a0522d',
    furDark: '#5e2609',
    furLight: '#d98152',
    belly: '#f5deb3',
    pink: '#e8b4b8',
    eyes: '#3d1a0a',
    eyeHighlight: '#ffffff',
    cheeks: '#d2828a',
    paws: '#eed9c4',
  },
  {
    id: 'lilac_mist',
    name: 'Дымчато-сиреневый',
    nameEn: 'Lilac Mist',
    description: 'Благородный пепельно-лавандовый оттенок с мягким градиентом.',
    fur: '#9d8189',
    furDark: '#5c464c',
    furLight: '#d1bcc2',
    belly: '#f4acb7',
    pink: '#ffcad4',
    eyes: '#2e1f27',
    eyeHighlight: '#ffffff',
    cheeks: '#e07a5f',
    paws: '#ffe5d9',
  },
  {
    id: 'silver_pearl',
    name: 'Серебристый жемчуг',
    nameEn: 'Silver Pearl',
    description: 'Светло-серый шелковистый окрас с мерцающей белой грудкой.',
    fur: '#b8c0c8',
    furDark: '#75808d',
    furLight: '#e4eaf0',
    belly: '#f0f4f8',
    pink: '#f9d2dc',
    eyes: '#222831',
    eyeHighlight: '#ffffff',
    cheeks: '#f3abbc',
    paws: '#e9ecef',
  },
  {
    id: 'blue_sapphire',
    name: 'Сапфировый джунгарик',
    nameEn: 'Blue Sapphire',
    description: 'Холодный серо-голубой окрас джунгарского хомячка.',
    fur: '#6c7d93',
    furDark: '#394657',
    furLight: '#a6b8d1',
    belly: '#dce4ec',
    pink: '#e6b8c2',
    eyes: '#1a2530',
    eyeHighlight: '#ffffff',
    cheeks: '#d48c9e',
    paws: '#cbd5e1',
  },
  {
    id: 'chocolate_brown',
    name: 'Шоколадный брауни',
    nameEn: 'Chocolate Brown',
    description: 'Глубокий цвет темного шоколада с карамельной грудкой.',
    fur: '#5c3a21',
    furDark: '#2e1808',
    furLight: '#99633b',
    belly: '#d4a373',
    pink: '#e29578',
    eyes: '#1e0f06',
    eyeHighlight: '#ffffff',
    cheeks: '#dd6b55',
    paws: '#e6ccb2',
  },
  {
    id: 'argente_red',
    name: 'Оранжево-песочный',
    nameEn: 'Argente Red',
    description: 'Яркий персиково-апельсиновый оттенок шерсти с карими глазами.',
    fur: '#e76f51',
    furDark: '#99351d',
    furLight: '#fca188',
    belly: '#ffe8d6',
    pink: '#f4a261',
    eyes: '#491b1d',
    eyeHighlight: '#ffffff',
    cheeks: '#e9c46a',
    paws: '#f8edeb',
  },
  {
    id: 'honey_gold',
    name: 'Медовый янтарь',
    nameEn: 'Honey Gold',
    description: 'Теплый солнечный янтарно-желтый цвет летнего меда.',
    fur: '#f4a261',
    furDark: '#a8571c',
    furLight: '#ffd4ab',
    belly: '#fff1d0',
    pink: '#fbc3bc',
    eyes: '#381e05',
    eyeHighlight: '#ffffff',
    cheeks: '#e76f51',
    paws: '#fde8cd',
  },
  {
    id: 'rust_orange',
    name: 'Рыжий кирпичный',
    nameEn: 'Rust Orange',
    description: 'Насыщенный осенний рыжий окрас пушистого зверька.',
    fur: '#d35400',
    furDark: '#7a2d00',
    furLight: '#ff9248',
    belly: '#fdebd0',
    pink: '#edbb99',
    eyes: '#2c1200',
    eyeHighlight: '#ffffff',
    cheeks: '#e59866',
    paws: '#fadbd8',
  },
  {
    id: 'dove_grey',
    name: 'Голубино-серый',
    nameEn: 'Dove Grey',
    description: 'Умиротворяющий матовый дымчатый цвет голубиного пера.',
    fur: '#8d99ae',
    furDark: '#4a5363',
    furLight: '#c2cee3',
    belly: '#edf2f4',
    pink: '#e8b4b8',
    eyes: '#2b2d42',
    eyeHighlight: '#ffffff',
    cheeks: '#d98894',
    paws: '#d8e2dc',
  },
  {
    id: 'mink_soft',
    name: 'Норковый бежевый',
    nameEn: 'Mink Soft',
    description: 'Элегантный бежевый цвет шелковой норки с мягким свечением.',
    fur: '#b79ced',
    furDark: '#6c4baf',
    furLight: '#dfd1ff',
    belly: '#f3ecff',
    pink: '#f8bbd0',
    eyes: '#281a40',
    eyeHighlight: '#ffffff',
    cheeks: '#ea80fc',
    paws: '#ede7f6',
  },
  {
    id: 'copper_glow',
    name: 'Медно-красный',
    nameEn: 'Copper Glow',
    description: 'Сияющий металлический медно-бронзовый цвет шерстки.',
    fur: '#b85d38',
    furDark: '#692b11',
    furLight: '#f29672',
    belly: '#fad3b8',
    pink: '#e69c8a',
    eyes: '#3b160a',
    eyeHighlight: '#ffffff',
    cheeks: '#cc5a43',
    paws: '#f5c6aa',
  },
  {
    id: 'ivory_white',
    name: 'Слоновая кость',
    nameEn: 'Ivory White',
    description: 'Мягкий сливочно-белый оттенок с темными глазками.',
    fur: '#fefae0',
    furDark: '#b8b082',
    furLight: '#ffffff',
    belly: '#ffffff',
    pink: '#fbc4ab',
    eyes: '#1e1e1e',
    eyeHighlight: '#ffffff',
    cheeks: '#f8ad9d',
    paws: '#faedcd',
  },
  {
    id: 'obsidian_onyx',
    name: 'Глянцевый оникс',
    nameEn: 'Obsidian Onyx',
    description: 'Смоляной черный мех с белыми перчатками на лапках.',
    fur: '#1a1a1e',
    furDark: '#08080a',
    furLight: '#464652',
    belly: '#28282e',
    pink: '#ff9ebb',
    eyes: '#000000',
    eyeHighlight: '#00e5ff',
    cheeks: '#e91e63',
    paws: '#ffffff',
  },
  {
    id: 'cyber_rainbow',
    name: 'Party Cyber / Неон',
    nameEn: 'Party Cyber / Neon',
    description: 'Секретный киберпанк окрас с неоновой бирюзой и фуксией.',
    fur: '#00f5d4',
    furDark: '#0077b6',
    furLight: '#80fff0',
    belly: '#fee440',
    pink: '#f15bb5',
    eyes: '#9b5de5',
    eyeHighlight: '#ffffff',
    cheeks: '#ff006e',
    paws: '#00f5d4',
  },
];

/**
 * 24x24 МАТРИЦЫ ХОМЯКА-БУХАНОЧКИ (Широкий, приземистый пухлый овал)
 * Ширина тела ~22px, высота всего ~12-14px!
 */

// 1. Спокойно сидит / IDLE 1 (пухлый горизонтальный овальчик, дышит)
export const HAMSTER_24_IDLE_1 = [
  '........................',
  '........................',
  '........................',
  '........................',
  '........................',
  '........................',
  '................DDDD....',
  '...............DPPPDD...',
  '...........DDDD.DPPPFD..',
  '..........DPPPDD.DFFFD..',
  '.....DDDD..DPPFDD.DDD...',
  '...DDFFFFDD.DFFFFFFFFD..',
  '..DFFFFFFFFDFFFFFFFFFFD.',
  '.DFFFFFFFFLLFFFFEEHFFFD.',
  'DFFFFFFFFFLLFFFFEEHFFFPD',
  'DFFFFFFFFFFFFFFFFFDFFCFD',
  'DFFFFFFFFFFBBBBBBBDDDCFD',
  'DFFFFFFFFFBBBBBBBBBBFFD.',
  '.DFFFFFFFBBBBBBBBBBBFFD.',
  '..DFFFFFFFFFFFFFFFFFFD..',
  '...DFFD..DPPD...DPPD....',
  '....DPPD..DPD....DPD....',
  '........................',
  '........................',
];

// 2. Спокойно сидит / IDLE 2 (моргает глазками)
export const HAMSTER_24_IDLE_2 = [
  '........................',
  '........................',
  '........................',
  '........................',
  '........................',
  '........................',
  '...............DDDD.....',
  '..............DPPPDD....',
  '..........DDDD.DPPPFD...',
  '.........DPPPDD.DFFFD...',
  '.....DDDD.DPPFDD.DDD....',
  '...DDFFFFDD.DFFFFFFFFD..',
  '..DFFFFFFFFDFFFFFFFFFFD.',
  '.DFFFFFFFFLLFFFFDDFFFFD.',
  'DFFFFFFFFFLLFFFFDDFFFFPD',
  'DFFFFFFFFFFFFFFFFFDFFCFD',
  'DFFFFFFFFFFBBBBBBBDDDCFD',
  'DFFFFFFFFFBBBBBBBBBBFFD.',
  '.DFFFFFFFBBBBBBBBBBBFFD.',
  '..DFFFFFFFFFFFFFFFFFFD..',
  '...DFFD..DPPD...DPPD....',
  '....DPPD..DPD....DPD....',
  '........................',
  '........................',
];

// 3. Ходьба 1 / WALK 1 (переступает лапками вперевалку)
export const HAMSTER_24_WALK_1 = [
  '........................',
  '........................',
  '........................',
  '........................',
  '........................',
  '........................',
  '................DDDD....',
  '...............DPPPDD...',
  '...........DDDD.DPPPFD..',
  '..........DPPPDD.DFFFD..',
  '.....DDDD..DPPFDD.DDD...',
  '...DDFFFFDD.DFFFFFFFFD..',
  '..DFFFFFFFFDFFFFFFFFFFD.',
  '.DFFFFFFFFLLFFFFEEHFFFD.',
  'DFFFFFFFFFLLFFFFEEHFFFPD',
  'DFFFFFFFFFFFFFFFFFDFFCFD',
  'DFFFFFFFFFFBBBBBBBDDDCFD',
  'DFFFFFFFFFBBBBBBBBBBFFD.',
  '..DFFFFFFBBBBBBBBBBBFFD.',
  '...DFFFFFFFFFFFFFFFFFD..',
  '...DPPD...DPPD...DFFD...',
  '....DPD....DPD..DPPPD...',
  '.................DPD....',
  '........................',
];

// 4. Ходьба 2 / WALK 2 (нейтральная фаза шага)
export const HAMSTER_24_WALK_2 = [
  '........................',
  '........................',
  '........................',
  '........................',
  '........................',
  '........................',
  '................DDDD....',
  '...............DPPPDD...',
  '...........DDDD.DPPPFD..',
  '..........DPPPDD.DFFFD..',
  '.....DDDD..DPPFDD.DDD...',
  '...DDFFFFDD.DFFFFFFFFD..',
  '..DFFFFFFFFDFFFFFFFFFFD.',
  '.DFFFFFFFFLLFFFFEEHFFFD.',
  'DFFFFFFFFFLLFFFFEEHFFFPD',
  'DFFFFFFFFFFFFFFFFFDFFCFD',
  'DFFFFFFFFFFBBBBBBBDDDCFD',
  'DFFFFFFFFFBBBBBBBBBBFFD.',
  '.DFFFFFFFBBBBBBBBBBBFFD.',
  '..DFFFFFFFFFFFFFFFFFFD..',
  '...DFFD..DPPD...DPPD....',
  '....DPPD..DPD....DPD....',
  '........................',
  '........................',
];

// 5. Ходьба 3 / WALK 3 (другая пара лапок шагает)
export const HAMSTER_24_WALK_3 = [
  '........................',
  '........................',
  '........................',
  '........................',
  '........................',
  '........................',
  '................DDDD....',
  '...............DPPPDD...',
  '...........DDDD.DPPPFD..',
  '..........DPPPDD.DFFFD..',
  '.....DDDD..DPPFDD.DDD...',
  '...DDFFFFDD.DFFFFFFFFD..',
  '..DFFFFFFFFDFFFFFFFFFFD.',
  '.DFFFFFFFFLLFFFFEEHFFFD.',
  'DFFFFFFFFFLLFFFFEEHFFFPD',
  'DFFFFFFFFFFFFFFFFFDFFCFD',
  'DFFFFFFFFFFBBBBBBBDDDCFD',
  'DFFFFFFFFFBBBBBBBBBBFFD.',
  '..DFFFFFFBBBBBBBBBBBFFD.',
  '...DFFFFFFFFFFFFFFFFFD..',
  '.....DFFD...DPPD..DPPD..',
  '....DPPPD..DPPPD...DPD..',
  '.....DPD....DPD.........',
  '........................',
];

// 6. Ходьба 4 / WALK 4
export const HAMSTER_24_WALK_4 = HAMSTER_24_WALK_2;

// 7. Бег в колесе 1 / WHEEL 1 (вытянутый в беге горизонтальный овал)
export const HAMSTER_24_WHEEL_1 = [
  '........................',
  '........................',
  '........................',
  '........................',
  '........................',
  '........................',
  '...............DDDD.....',
  '..............DPPPDD....',
  '..........DDDD.DPPPFD...',
  '.........DPPPDD.DFFFD...',
  '..DDDFFFFF.DPPFDD.DDD...',
  '.DFFFFFFFFD.DFFFFFFFFD..',
  'DFFFFFFFFFLLFFFFEEHFFFD.',
  'DFFFFFFFFFLLFFFFEEHFFFPD',
  'DFFFFFFFFFFFFFFFFFDFFCFD',
  '.DFFFFFFFFFBBBBBBBDDDCFD',
  '..DFFFFFFBBBBBBBBBBBFFD.',
  '...DFFFFFFFFFFFFFFFFFD..',
  '..DPPD.DFFD....DPPD.DPD.',
  '.DPPPD..DPD...DPPPD.DPPD',
  '........................',
  '........................',
  '........................',
  '........................',
];

// 8. Бег в колесе 2 / WHEEL 2 (лапки сгруппированы под пузиком)
export const HAMSTER_24_WHEEL_2 = [
  '........................',
  '........................',
  '........................',
  '........................',
  '........................',
  '........................',
  '................DDDD....',
  '...............DPPPDD...',
  '...........DDDD.DPPPFD..',
  '..........DPPPDD.DFFFD..',
  '..DDDFFFFF.DPPFDD.DDD...',
  '.DFFFFFFFFD.DFFFFFFFFD..',
  'DFFFFFFFFFLLFFFFEEHFFFD.',
  'DFFFFFFFFFLLFFFFEEHFFFPD',
  'DFFFFFFFFFFFFFFFFFDFFCFD',
  '.DFFFFFFFFFBBBBBBBDDDCFD',
  '..DFFFFFFBBBBBBBBBBBFFD.',
  '...DFFFFFFFFFFFFFFFFFD..',
  '....DFFDDPPD..DFFDDPPD..',
  '....DPPPDDPD..DPPPDDPD..',
  '........................',
  '........................',
  '........................',
  '........................',
];

// 9. Умывание 1 / GROOM 1 (присел на задние лапки, моет щечки)
export const HAMSTER_24_GROOM_1 = [
  '........................',
  '........................',
  '........................',
  '........................',
  '........................',
  '........................',
  '................DDDD....',
  '...............DPPPDD...',
  '...........DDDD.DPPPFD..',
  '..........DPPPDD.DFFFD..',
  '.....DDDD..DPPFDD.DDD...',
  '...DDFFFFDD.DFFFFFFFFD..',
  '..DFFFFFFFFDFFFFFFFFFFD.',
  '.DFFFFFFFFLLFFFFDDFFFFD.',
  'DFFFFFFFFFLLFFFDPPDFFFPD',
  'DFFFFFFFFFFFFFDPPPDCFFD.',
  'DFFFFFFFFFFBBBDDPPDDCFFD',
  'DFFFFFFFFFBBBBBBBBBBFFD.',
  '.DFFFFFFFBBBBBBBBBBBFFD.',
  '..DFFFFFFFFFFFFFFFFFFD..',
  '...DFFD..DPPD...DPPD....',
  '....DPPD..DPD....DPD....',
  '........................',
  '........................',
];

// 10. Умывание 2 / GROOM 2 (моет ушки)
export const HAMSTER_24_GROOM_2 = [
  '........................',
  '........................',
  '........................',
  '........................',
  '........................',
  '........................',
  '...............DPPPDD...',
  '...........DDDDPPPDDD...',
  '..........DPPPDDPPPFD...',
  '.....DDDD..DPPFD.DFFD...',
  '...DDFFFFDD.DDD..DDD....',
  '..DFFFFFFFFDFFFFFFFFFFD.',
  '.DFFFFFFFFLLFFFFDDFFFFD.',
  'DFFFFFFFFFLLFFFFDDFFFFPD',
  'DFFFFFFFFFFFFFFFFFDFFCFD',
  'DFFFFFFFFFFBBBBBBBDDDCFD',
  'DFFFFFFFFFBBBBBBBBBBFFD.',
  '.DFFFFFFFBBBBBBBBBBBFFD.',
  '..DFFFFFFFFFFFFFFFFFFD..',
  '...DFFD..DPPD...DPPD....',
  '....DPPD..DPD....DPD....',
  '........................',
  '........................',
  '........................',
];

// 11. Обнюхивание / SNIFF (носом к опилкам)
export const HAMSTER_24_SNIFF_1 = [
  '........................',
  '........................',
  '........................',
  '........................',
  '........................',
  '........................',
  '........................',
  '................DDDD....',
  '...............DPPPDD...',
  '...........DDDD.DPPPFD..',
  '.....DDDD..DPPFDD.DDD...',
  '...DDFFFFDD.DFFFFFFFFD..',
  '..DFFFFFFFFDFFFFFFFFFFD.',
  '.DFFFFFFFFLLFFFFEEHFFFD.',
  'DFFFFFFFFFLLFFFFEEHFFFPD',
  'DFFFFFFFFFFFFFFFFFDFFCFD',
  'DFFFFFFFFFFBBBBBBBDDDCFD',
  'DFFFFFFFFFBBBBBBBBDPPD..',
  '.DFFFFFFFBBBBBBBBBDDPD..',
  '..DFFFFFFFFFFFFFFFFFFD..',
  '...DFFD..DPPD...DPPD....',
  '....DPPD..DPD....DPD....',
  '........................',
  '........................',
];

// 12. Кушает 1 / EAT 1 (держит зернышко 'S')
export const HAMSTER_24_EAT_1 = [
  '........................',
  '........................',
  '........................',
  '........................',
  '........................',
  '........................',
  '................DDDD....',
  '...............DPPPDD...',
  '...........DDDD.DPPPFD..',
  '..........DPPPDD.DFFFD..',
  '.....DDDD..DPPFDD.DDD...',
  '...DDFFFFDD.DFFFFFFFFD..',
  '..DFFFFFFFFDFFFFFFFFFFD.',
  '.DFFFFFFFFLLFFFFEEHFFFD.',
  'DFFFFFFFFFLLFFFFEEHFFFPD',
  'DFFFFFFFFFFFFFSSFFDFFCFD',
  'DFFFFFFFFFFBBBSSSSDDDCFD',
  'DFFFFFFFFFBBBBDPPPDDFFD.',
  '.DFFFFFFFBBBBBBBBBBBFFD.',
  '..DFFFFFFFFFFFFFFFFFFD..',
  '...DFFD..DPPD...DPPD....',
  '....DPPD..DPD....DPD....',
  '........................',
  '........................',
];

// 13. Кушает 2 / EAT 2 (жует)
export const HAMSTER_24_EAT_2 = [
  '........................',
  '........................',
  '........................',
  '........................',
  '........................',
  '........................',
  '................DDDD....',
  '...............DPPPDD...',
  '...........DDDD.DPPPFD..',
  '..........DPPPDD.DFFFD..',
  '.....DDDD..DPPFDD.DDD...',
  '...DDFFFFDD.DFFFFFFFFD..',
  '..DFFFFFFFFDFFFFFFFFFFD.',
  '.DFFFFFFFFLLFFFFEEHFFFD.',
  'DFFFFFFFFFLLFFFFEEHFFFPD',
  'DFFFFFFFFFFFFF.SFFDFFCFD',
  'DFFFFFFFFFFBBB.SSSDDCFFD',
  'DFFFFFFFFFBBBBDPPPDDFFD.',
  '.DFFFFFFFBBBBBBBBBBBFFD.',
  '..DFFFFFFFFFFFFFFFFFFD..',
  '...DFFD..DPPD...DPPD....',
  '....DPPD..DPD....DPD....',
  '........................',
  '........................',
];

// 14. Спит 1 / SLEEP 1 (полная распластанная лепешка / loaf sleeping)
export const HAMSTER_24_SLEEP_1 = [
  '........................',
  '........................',
  '........................',
  '........................',
  '........................',
  '........................',
  '........................',
  '........................',
  '........................',
  '........................',
  '.....DDDDDDDDDDDDD......',
  '..DDDFFFFFFFFFFFFFDDD...',
  '.DFFFFFFFFLLFFFFFFFFFD..',
  'DFFFFFFFFFLLFFFFFFFFFFD.',
  'DFFFFFFFFFFFFFFFFDDPPFD.',
  'DFFFFFFFFFFFFFFFFDDDDFFD',
  'DFFFFFFFFFFBBBBBBBBBBFFD',
  '.DFFFFFFFFBBBBBBBBBBBFFD',
  '..DFFFFFFFFFFFFFFFFFFD..',
  '....DDDDDDDDDDDDDDDDD...',
  '........................',
  '........................',
  '........................',
  '........................',
];

// 15. Спит 2 / SLEEP 2 (вдох - чуть надулся)
export const HAMSTER_24_SLEEP_2 = [
  '........................',
  '........................',
  '........................',
  '........................',
  '........................',
  '........................',
  '........................',
  '........................',
  '........................',
  '.....DDDDDDDDDDDDD......',
  '..DDDFFFFFFFFFFFFFDDD...',
  '.DFFFFFFFFLLFFFFFFFFFD..',
  'DFFFFFFFFFLLFFFFFFFFFFD.',
  'DFFFFFFFFFFFFFFFFFFFFFD.',
  'DFFFFFFFFFFFFFFFFDDPPFD.',
  'DFFFFFFFFFFFFFFFFDDDDFFD',
  'DFFFFFFFFFFBBBBBBBBBBFFD',
  '.DFFFFFFFFBBBBBBBBBBBFFD',
  '..DFFFFFFFFFFFFFFFFFFD..',
  '....DDDDDDDDDDDDDDDDD...',
  '........................',
  '........................',
  '........................',
  '........................',
];

// 16. Лежит / LAYING (расслабленная булочка)
export const HAMSTER_24_LAYING = [
  '........................',
  '........................',
  '........................',
  '........................',
  '........................',
  '........................',
  '........................',
  '........................',
  '................DDDD....',
  '...............DPPPDD...',
  '...........DDDD.DPPPFD..',
  '..DDDDDDDD..DPPFDD.DDD..',
  '.DFFFFFFFFD.DFFFFFFFFD..',
  'DFFFFFFFFFLLFFFFEEHFFFD.',
  'DFFFFFFFFFLLFFFFEEHFFFPD',
  'DFFFFFFFFFFFFFFFFFDFFCFD',
  'DFFFFFFFFFFBBBBBBBDDDCFD',
  'DFFFFFFFFFBBBBBBBBBBFFD.',
  '.DFFFFFFBBBBBBBBBBBBFFD.',
  '..DPPDFFFFFFFFFFFFFFD...',
  '...DPPDDDDDDDDDDPPD.....',
  '........................',
  '........................',
  '........................',
];

// 17. Дефекация / POOPING
export const HAMSTER_24_POOPING = [
  '........................',
  '........................',
  '........................',
  '........................',
  '........................',
  '........................',
  '................DDDD....',
  '...............DPPPDD...',
  '...........DDDD.DPPPFD..',
  '..........DPPPDD.DFFFD..',
  '.....DDDD..DPPFDD.DDD...',
  '...DDFFFFDD.DFFFFFFFFD..',
  '..DFFFFFFFFDFFFFFFFFFFD.',
  '.DFFFFFFFFLLFFFFEEHFFFD.',
  'DFFFFFFFFFLLFFFFEEHFFFPD',
  'DFFFFFFFFFFFFFFFFFDFFCFD',
  'DFFFFFFFFFFBBBBBBBDDDCFD',
  'DFFFFFFFFFFFFFFFFFFFFFD.',
  '.DFFFFFFFFFFFFFFFFFFFDD.',
  '..DFFFFFFFFFFFFFFFFFFD..',
  '...DFFD..DPPD...DPPD....',
  '....DPPD..DPD....DPD....',
  '........................',
  '........................',
];

// Алиасы обратной совместимости
export const HAMSTER_IDLE_FRAME_1 = HAMSTER_24_IDLE_1;
export const HAMSTER_IDLE_FRAME_2 = HAMSTER_24_IDLE_2;
export const HAMSTER_WALK_FRAME_1 = HAMSTER_24_WALK_1;
export const HAMSTER_WALK_FRAME_2 = HAMSTER_24_WALK_2;
export const HAMSTER_LAYING_FRAME = HAMSTER_24_LAYING;
export const HAMSTER_SLEEP_FRAME_1 = HAMSTER_24_SLEEP_1;
export const HAMSTER_SLEEP_FRAME_2 = HAMSTER_24_SLEEP_2;
export const HAMSTER_EATING_FRAME_1 = HAMSTER_24_EAT_1;
export const HAMSTER_EATING_FRAME_2 = HAMSTER_24_EAT_2;
export const HAMSTER_POOPING_FRAME = HAMSTER_24_POOPING;

export const FOOD_ITEMS: FoodItem[] = [
  {
    id: 'sunflower_seeds',
    name: 'Семечки подсолнуха',
    description: 'Хрустящее любимое лакомство. Быстро съедается.',
    icon: '🌻',
    hungerGain: 15,
    happinessGain: 10,
    healthGain: 2,
    eatingDurationSec: 3,
  },
  {
    id: 'grain_mix',
    name: 'Зерновая смесь',
    description: 'Овсяные хлопья, просо и пшеница. Сытный сбалансированный обед.',
    icon: '🌾',
    hungerGain: 35,
    happinessGain: 15,
    healthGain: 5,
    eatingDurationSec: 5,
  },
  {
    id: 'apple_slice',
    name: 'Сочное яблоко',
    description: 'Свежий ломтик фрукта. Богат витаминами и отлично поднимает настроение.',
    icon: '🍎',
    hungerGain: 25,
    happinessGain: 25,
    healthGain: 10,
    eatingDurationSec: 4,
  },
  {
    id: 'drops_treat',
    name: 'Йогуртовые дропсы',
    description: 'Царское лакомство для грызунов. Максимальное счастье и целебный эффект!',
    icon: '🍓',
    hungerGain: 45,
    happinessGain: 40,
    healthGain: 20,
    eatingDurationSec: 6,
  },
];
